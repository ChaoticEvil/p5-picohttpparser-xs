...
static void normalize_response_header_name(pTHX_
                                           char* const dest,
...
static void concat_multiline_header(pTHX_ SV * val, const char * const cont, size_t const cont_len) {
    sv_catpvs(val, "\n"); /* XXX: is it collect? */
...
static int store_url_decoded(HV* env, const char* name, size_t name_len,
...
MODULE = PicoHttpParser::XS		PACKAGE = PicoHttpParser::XS

int parse_request(SV* buf, SV* envref)
PROTOTYPE: $$
CODE:
{
    const char* buf_str; STRLEN buf_len;
    ...
    if (!SvROK(envref))
        Perl_croak(aTHX_ "second param to parse_request should be a hashref");
...
void parse_response(SV* buf, int header_format, HV* special_headers = NULL)
PPCODE:
{
